...
{
  public class TXLiteVideoView : Panel
  {
    ...
    public bool RegEngine(string userId, TRTCVideoStreamType type, ITRTCCloud engine, bool local = false)
    {
      ...
      mUserId = userId;
      mStreamType = type;
      int count = TXLiteVideoViewManager.GetInstance().Count;
      if (engine != null)
      {
        if (count == 0)
        {
          engine.setLocalVideoRenderCallback(TRTCVideoPixelFormat.TRTCVideoPixelFormat_BGRA32,
              TRTCVideoBufferType.TRTCVideoBufferType_Buffer, TXLiteVideoViewManager.GetInstance());
        }
        if (!mLocalView)
        {
      ...
      if (mLocalView)
        TXLiteVideoViewManager.GetInstance().AddView("", type, this);
      else
        TXLiteVideoViewManager.GetInstance().AddView(userId, type, this);
      lock (mArgbFrame)
        ReleaseBuffer(mArgbFrame);
      mOccupy = true;
      this.Refresh();
    ...
    {
      if (mLocalView)
        TXLiteVideoViewManager.GetInstance().RemoveView("", mStreamType, this);
      else
        TXLiteVideoViewManager.GetInstance().RemoveView(mUserId, mStreamType, this);
      int count = TXLiteVideoViewManager.GetInstance().Count;
      if (engine != null)
      {
        if (count == 0)
        ...
        if (!mLocalView && !TXLiteVideoViewManager.GetInstance().HasUserId(mUserId))
        {
    ...
    {
      TXLiteVideoViewManager.GetInstance().RemoveAllView();
    }
  ...
  class TXLiteVideoViewManager : ITRTCVideoRenderCallback
  {
    private volatile Dictionary<string, TXLiteVideoView> mMapViews;

    public static TXLiteVideoViewManager sInstance;

    private static Object mLocker = new Object();

    public static TXLiteVideoViewManager GetInstance()
    {
      if (sInstance == null)
      {
        lock (mLocker)
        {
          if (sInstance == null)
            sInstance = new TXLiteVideoViewManager();
        }
      }
      return sInstance;
    }

    private TXLiteVideoViewManager()
    {
      mMapViews = new Dictionary<string, TXLiteVideoView>();
    }
    ...
    public void AddView(string userId, TRTCVideoStreamType type, TXLiteVideoView view)
    {
      lock (mMapViews)
      {
        bool find = false;
    ...
    public void RemoveView(string userId, TRTCVideoStreamType type, TXLiteVideoView view)
    {
      lock (mMapViews)
      {
